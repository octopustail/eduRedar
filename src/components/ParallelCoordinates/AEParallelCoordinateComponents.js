...
export default class ParallelGraph extends Component {
    ...
    dataFormate=(data)=>{
        ...
        d.forEach((item)=>{
            //parseInt应该在父亲函数位置处理。
            dataGroup.push([parseFloat(item["ae1s"]),parseFloat(item["ae2s"]),parseFloat(item["ae3s"]),parseFloat(item["ae4s"]),parseFloat(item["ae5s"]),parseFloat(item["ae6s"])])
        })
        return dataGroup
    ...
    getOption = (ori_data) => {
        ...
        return {
            ...
            parallelAxis: [
                { dim: 0, name: schema[0].text,max:4,min:0},
                { dim: 1, name: schema[1].text,max:4,min:0},
                { dim: 2, name: schema[2].text,max:4,min:0},
                { dim: 3, name: schema[3].text,max:4,min:0},
                { dim: 4, name: schema[4].text,max:4,min:0},
                { dim: 5, name: schema[5].text,max:4,min:0},
    ...
    render() {
        ...
        return (
            <div className="parallel">
                <ReactEcharts option = {this.getOption(this.props.data)} style = {{height:500,width:600}} ref = {e=>{this.echarts_react = e}}/>
            </div>
        )
    }
}
